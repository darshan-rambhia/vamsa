...
function ActivityComponent() {
  ...
  const activity = useQuery(api.auditLogs.recentActivity, { token, limit: 50 });
        ...
        <CardContent className="py-6">
          {activity === undefined ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="font-display text-lg text-foreground mb-2">No activity yet</h3>
...
function ActivityItem({ item, isFirst }: ActivityItemProps) {
  ...
  const colorClass = actionColors[item.actionType as keyof typeof actionColors] || "bg-muted text-muted-foreground";
  ...
  if (isPersonActivity) {
    return (
      <Link
        ...
        className="block p-2 -mx-2 rounded-lg hover:bg-accent transition-colors"
      >
        {content}
      </Link>
    );
