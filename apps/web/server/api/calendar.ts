 ...
 */
calendarRouter.get("/rss.xml", async (c) => {
  try {
    ...
    const feed = new RSS({
      ...
      feed_url: `${appUrl}/api/v1/calendar/rss.xml`,
      site_url: appUrl,
    ...
    // Get recent audit logs for feed items
 ...
 */
calendarRouter.get("/birthdays.ics", async (c) => {
  try {
    const appUrl = process.env.APP_URL || "http://localhost:5173";
    const token = c.req.query("token");

    // If token is provided, validate it (optional for public calendar)
    if (token) {
 ...
 * Anniversary calendar (wedding and memorial dates) in iCalendar format
 */
calendarRouter.get("/anniversaries.ics", async (c) => {
  try {
    const appUrl = process.env.APP_URL || "http://localhost:5173";
    const token = c.req.query("token");
    ...
    // Get memorial dates (death anniversaries)
 ...
 */
calendarRouter.get("/events.ics", async (c) => {
  try {
    const appUrl = process.env.APP_URL || "http://localhost:5173";
    const token = c.req.query("token");
    ...
    for (const event of events) {
      ...
      try {
        calendar.createEvent({
          ...
          start: eventDate,
          end: new Date(eventDate.getTime() + 24 * 60 * 60 * 1000), // All-day event
          allDay: true,
