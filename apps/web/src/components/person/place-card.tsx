    ...
    <div className="flex items-start justify-between gap-4">
      ...
      <div className="flex-1 space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {place.type && (
            <Badge variant="outline" className={getPlaceTypeColor(place.type)}>
  ...
  if (onClick) {
    return (
      <button
        type="button"
        onClick={onClick}
        className="border-border hover:border-primary/30 hover:bg-accent/5 focus-visible:ring-ring w-full cursor-pointer rounded-md border p-4 text-left transition-all duration-200 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none"
        aria-label={`View details for ${place.place.name}`}
      >
        {content}
      </button>
    );
