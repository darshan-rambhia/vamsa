...
/**
 * Calculates positions for all nodes in ancestor layout
 */
function calculateAncestorLayout(
  nodes: ChartNode[],
  edges: ChartEdge[],
  width: number
): Map<string, Position> {
...
function AncestorChartComponent({
  ...
  const nodePositions = useMemo(() => {
    ...
    const positions = calculateAncestorLayout(nodes, edges, dimensions.width);
    const endTime = performance.now();
    if (process.env.NODE_ENV === "development") {
      // eslint-disable-next-line no-console
      console.log(
        `[Performance] AncestorChart layout calculated in ${(endTime - startTime).toFixed(2)}ms`
      );
    }
    return positions;
