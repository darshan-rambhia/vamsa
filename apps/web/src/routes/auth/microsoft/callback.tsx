...
import { z } from "zod";
import { handleMicrosoftCallback } from "~/server/auth-oidc";
import {
  Card,
  CardContent,
  CardDescription,
...
function MicrosoftCallbackComponent() {
  ...
  useEffect(() => {
    async function processCallback() {
      ...
      try {
        const result = await handleMicrosoftCallback({ data: { code, state } });
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription className="mt-2">
            Please wait while we complete your Microsoft sign-in.
          </CardDescription>
        </CardHeader>
      </Card>
    </div>
